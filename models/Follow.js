...
let Follow = function (followedUsename, authorId) {
    this.followedUsename = followedUsename
    this.authorId = authorId
    this.errors = []
}

Follow.prototype.cleanUp = async function () {
    if (typeof (this.followedUsename) != 'string') {
        this.followedUsename = ''
    }
}

Follow.prototype.validate = async function (action) {
    // followed username must exist in database
    let followedAccount = await usersCollection.findOne({ username: this.followedUsename })
    if (followedAccount) {
    ...
    if (action == 'create') {
        if (doesFollowAlreadyExist) {
            this.errors.push('Already following this user.')
        }
    }
    if (action == 'delete') {
        if (!doesFollowAlreadyExist) {
